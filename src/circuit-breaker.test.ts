 ...
 * before initDatabase, so it has to create the dir itself).
 *
 * Ported from upstream 336e01d.
 */
import fs from 'fs';
import os from 'os';
...
// vi.hoisted runs before any import — use require() to access Node builtins.
...
describe('enforceStartupBackoff — state transitions', () => {
  ...
  it('resetCircuitBreaker after a startup clears the counter for the next startup', async () => {
...
describe('enforceStartupBackoff — backoff schedule', () => {
   ...
   * produces in seconds]. priorAttempt=null = no file = first start.
   */
  const cases: Array<{
    label: string;
...
describe('enforceStartupBackoff — fresh install (DATA_DIR missing)', () => {
  /**
   * The breaker runs before initDatabase (which is what creates DATA_DIR).
   * On a fresh checkout the dir doesn't exist yet, so write() must create
   * it before writing the state file — otherwise the host crashes on first start.
   */
